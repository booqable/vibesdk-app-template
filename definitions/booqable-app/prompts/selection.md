...
- Iframe session bootstrap: Booqable embeds the app in an iframe with a signed
  `?token=` — the starter verifies it server-side and starts a session.
- OAuth: `/api/oauth/callback` exchanges Booqable's authorization code and
  stores tokens in a signed cookie.
- An authenticated proxy to the Booqable JSON:API: the frontend calls
  `booqableApi('/orders?...')` and the worker forwards it with the OAuth token.
