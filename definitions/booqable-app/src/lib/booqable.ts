 ...
 * through the worker's authenticated proxy.
 */
...
/**
 * Call once on app load. When the app runs inside the Booqable back office,
 * the iframe URL carries `?token=<JWT>` — exchange it for a session cookie.
 ...
 * Returns the parsed JSON:API document, or throws on failure.
 */
export async function booqableApi<T = any>(path: string, init: RequestInit = {}): Promise<T> {
    const response = await fetch(`/api/booqable/proxy${path}`, init);
